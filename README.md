...
Four columns flag **built-in** Responsible-AI / security capabilities. This is a
best-effort snapshot — features move fast, so verify against each project's current docs.
...
Cell values: ✅ built-in / first-class · 🟡 partial, basic, or via an official plugin/integration · — not built-in (still achievable with external components).
...
| [Microsoft Agent Framework](https://github.com/microsoft/agent-framework) | ![](https://img.shields.io/github/stars/microsoft/agent-framework?style=flat-square) | .NET/Python | ✅ | ✅ | ✅ | 🟡 | Unified successor to Semantic Kernel + AutoGen (Entra Agent ID) |
...
4. Rate the four governance columns ✅ / 🟡 / — per the [legend](#governance-capability-legend); prefer ✅ only for documented, built-in support.
